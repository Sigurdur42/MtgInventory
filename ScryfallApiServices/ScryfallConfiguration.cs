...
namespace ScryfallApiServices
{
    public class ScryfallConfiguration
    {
        ...
        public bool IsSetOutdated(DateTime? setUpdateDate) => DateTime.Now.AddDays(-1 * UpdateSetCacheInDays).Date > (setUpdateDate ?? DateTime.MinValue).Date;

        public bool IsCardOutdated(DateTime? setUpdateDate) => DateTime.Now.AddDays(-1 * UpdateCardCacheInDays).Date > (setUpdateDate ?? DateTime.MinValue).Date;

        public bool IsCardPriceOutdated(DateTime? setUpdateDate) => DateTime.Now.AddDays(-1 * UpdateCardCachePriceInDays).Date > (setUpdateDate ?? DateTime.MinValue).Date;
    }
}